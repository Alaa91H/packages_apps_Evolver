...
public class NetworkTraffic extends TextView implements TunerService.Tunable,
    ...
    private static final int LAYOUT_MODE_ADAPTIVE = 0;
    private static final int LAYOUT_MODE_STACKED = 1;

    private static final int ALIGNMENT_START = 0;
    private static final int ALIGNMENT_CENTER = 1;
    private static final int ALIGNMENT_END = 2;
    ...
    private static final String NETWORK_TRAFFIC_LAYOUT_MODE =
            "system:network_traffic_layout_mode";
    private static final String NETWORK_TRAFFIC_HIDE_UNITS =
            "system:network_traffic_hide_units";
    private static final String NETWORK_TRAFFIC_SPEED_TEXT_SIZE =
            "system:network_traffic_speed_text_size";
    private static final String NETWORK_TRAFFIC_UNIT_TEXT_SIZE =
            "system:network_traffic_unit_text_size";
    private static final String NETWORK_TRAFFIC_LINE_SPACING =
            "system:network_traffic_line_spacing";
    private static final String NETWORK_TRAFFIC_TEXT_ALIGNMENT =
            "system:network_traffic_text_alignment";
    ...
    private boolean mHideUnits;
    private int mLayoutMode = LAYOUT_MODE_STACKED;
    private int mSpeedTextSize = 7;
    private int mUnitTextSize = 6;
    private int mLineSpacingPercent = 95;
    private int mTextAlignment = ALIGNMENT_END;
        ...
        mTrafficHandler = new Handler(mContext.getMainLooper()) {
            ...
            private void displayStatsAndReschedule() {
                ...
                if (mEnabled && mIsActive) {
                    CharSequence output = "";
                    final boolean stacked = mLayoutMode == LAYOUT_MODE_STACKED
                            && showUpstream && showDownstream;
                    if (stacked) {
                        // Keep the order stable in every locale: upload is always the first
                        // visual line and download is always the second visual line.
                        output = TextUtils.concat(
                                formatOutput(mTxBytes, true),
                                "\n",
                                formatOutput(mRxBytes, true));
                        submode = MODE_UPSTREAM_AND_DOWNSTREAM;
                    } else if (showUpstream && showDownstream) {
                        // Preserve the legacy adaptive presentation as an explicit option.
                        if (mTxBytes > mRxBytes) {
                            output = formatOutput(mTxBytes, false);
                            submode = MODE_UPSTREAM_ONLY;
                        } else if (mTxBytes < mRxBytes) {
                            output = formatOutput(mRxBytes, false);
            ...
            private CharSequence formatOutput(long speed, boolean inlineUnit) {
                String unit;
                String formatSpeed;
                ...
                spanSpeedString.setSpan(
                        new AbsoluteSizeSpan(spToPx(mSpeedTextSize)),
                        0,
                        formatSpeed.length(),
                        Spanned.SPAN_INCLUSIVE_INCLUSIVE);

                if (mHideUnits) {
                    return spanSpeedString;
                }

                final SpannableString spanUnitString = new SpannableString(unit);
                spanUnitString.setSpan(
                        new AbsoluteSizeSpan(spToPx(mUnitTextSize)),
                        0,
                        unit.length(),
                        Spanned.SPAN_INCLUSIVE_INCLUSIVE);
    ...
    protected void onAttachedToWindow() {
        ...
        if (!mAttached) {
            ...
            tunerService.addTunable(this, NETWORK_TRAFFIC_LAYOUT_MODE);
            tunerService.addTunable(this, NETWORK_TRAFFIC_HIDE_UNITS);
            tunerService.addTunable(this, NETWORK_TRAFFIC_SPEED_TEXT_SIZE);
            tunerService.addTunable(this, NETWORK_TRAFFIC_UNIT_TEXT_SIZE);
            tunerService.addTunable(this, NETWORK_TRAFFIC_LINE_SPACING);
            tunerService.addTunable(this, NETWORK_TRAFFIC_TEXT_ALIGNMENT);
    ...
    private void setFixedWidth() {
        ...
        if (mVisible) {
            ...
            final int baseTextWidthDp = stacked
                    ? (mHideUnits ? 28 : 44)
                    : 18;
            float textScale = mSpeedTextSize / 7f;
            if (!mHideUnits) {
                textScale = Math.max(textScale, mUnitTextSize / 6f);
            }
            textScale = Math.max(0.75f, Math.min(1.75f, textScale));
            final int arrowWidthDp = mHideArrows ? 0 : 12;
            requiredWidth = (int) Math.ceil(
                    (baseTextWidthDp * textScale + arrowWidthDp)
    ...
    public void onTuningChanged(String key, String newValue) {
        switch (key) {
            ...
            case NETWORK_TRAFFIC_HIDEARROW:
                ...
                setFixedWidth();
                break;
            case NETWORK_TRAFFIC_LAYOUT_MODE:
                mLayoutMode = TunerService.parseInteger(newValue, LAYOUT_MODE_STACKED);
                if (mLayoutMode != LAYOUT_MODE_ADAPTIVE && mLayoutMode != LAYOUT_MODE_STACKED) {
                    mLayoutMode = LAYOUT_MODE_STACKED;
                }
                setTrafficDrawable();
                setFixedWidth();
                updateViews();
                break;
            case NETWORK_TRAFFIC_HIDE_UNITS:
                mHideUnits = TunerService.parseIntegerSwitch(newValue, false);
                setFixedWidth();
                updateViews();
                break;
            case NETWORK_TRAFFIC_SPEED_TEXT_SIZE:
                mSpeedTextSize = Math.max(5, Math.min(10,
                        TunerService.parseInteger(newValue, 7)));
                setFixedWidth();
                updateViews();
                break;
            case NETWORK_TRAFFIC_UNIT_TEXT_SIZE:
                mUnitTextSize = Math.max(4, Math.min(9,
                        TunerService.parseInteger(newValue, 6)));
                setFixedWidth();
                updateViews();
                break;
            case NETWORK_TRAFFIC_LINE_SPACING:
                mLineSpacingPercent = Math.max(70, Math.min(120,
                        TunerService.parseInteger(newValue, 95)));
                applyTextAppearance();
                updateViews();
                break;
            case NETWORK_TRAFFIC_TEXT_ALIGNMENT:
                mTextAlignment = Math.max(ALIGNMENT_START, Math.min(ALIGNMENT_END,
                        TunerService.parseInteger(newValue, ALIGNMENT_END)));
                applyTextAppearance();
                updateViews();
                break;
            default:
    ...
    private void applyTextAppearance() {
        setLines(2);
        setMaxLines(2);
        final String textFont = getResources().getString(
                com.android.internal.R.string.config_bodyFontFamily);
        setTypeface(Typeface.create(textFont, Typeface.BOLD));
        setLineSpacing(0f, Math.max(0.70f, Math.min(1.20f,
                mLineSpacingPercent / 100f)));
        setLayoutDirection(View.LAYOUT_DIRECTION_LOCALE);

        // Network rates are numeric/Latin tokens. Keep their internal order stable while the
        // view itself mirrors naturally through locale-aware layout and relative gravity.
        setTextDirection(View.TEXT_DIRECTION_FIRST_STRONG_LTR);

        if (mTextAlignment == ALIGNMENT_START) {
            setTextAlignment(View.TEXT_ALIGNMENT_VIEW_START);
            setGravity(Gravity.START | Gravity.CENTER_VERTICAL);
        } else if (mTextAlignment == ALIGNMENT_CENTER) {
    ...
    private int spToPx(int valueSp) {
