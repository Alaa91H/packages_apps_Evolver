...
class ModernStatusBarMobileView(context: Context, attrs: AttributeSet?) :
    ...
    private fun applyMobileTypePresentation() {
        val mobileGroup = requireViewById<AlphaOptimizedLinearLayout>(R.id.mobile_group)
        val activityContainer = requireViewById<View>(R.id.inout_container)
        val presentation =
            requireViewById<FrameLayout>(R.id.mobile_type_presentation_container)
        val typeContainer = requireViewById<FrameLayout>(R.id.mobile_type_container)
        val typeView = requireViewById<ImageView>(R.id.mobile_type)
        val signalContainer = requireViewById<FrameLayout>(R.id.mobile_signal_container)
        val signalView = requireViewById<ImageView>(R.id.mobile_signal)
        val mobileHd = requireViewById<ImageView>(R.id.mobile_hd)

        val useCompact = mobileTypeCustomizationEnabled && compactMobileType && !hideMobileType

        if (useCompact) {
            val compactTypeHeight = compactMobileTypeHeight()
            val signalHeight = mobileSignalHeight()
            ...
            signalContainer.addView(presentation, compactParams)

            typeContainer.layoutParams =
                FrameLayout.LayoutParams(
                    ViewGroup.LayoutParams.WRAP_CONTENT,
            ...
            typeView.layoutParams =
                typeView.layoutParams.apply {
                    height = compactTypeHeight
                }

            (signalContainer.layoutParams as LinearLayout.LayoutParams).apply {
                width = ViewGroup.LayoutParams.WRAP_CONTENT
                height = signalHeight + compactTypeHeight
                gravity = Gravity.CENTER_VERTICAL
                signalContainer.layoutParams = this
            }
            (signalView.layoutParams as FrameLayout.LayoutParams).apply {
                gravity = Gravity.BOTTOM or Gravity.CENTER_HORIZONTAL
                signalView.layoutParams = this
            }
            (mobileHd.layoutParams as FrameLayout.LayoutParams).apply {
                gravity = Gravity.START or Gravity.BOTTOM
                topMargin = 0
                mobileHd.layoutParams = this
            }
        } else {
            (presentation.parent as? ViewGroup)?.removeView(presentation)
            val standardParams = standardMobileTypeLayoutParams()
            val insertIndex = (mobileGroup.indexOfChild(activityContainer) + 1).coerceAtLeast(0)
            mobileGroup.addView(presentation, insertIndex, standardParams)
            ...
            typeView.layoutParams =
                typeView.layoutParams.apply {
                    height = standardMobileTypeHeight()
                }
        ...
        presentation.visibility =
            if (mobileTypeCustomizationEnabled && hideMobileType) View.GONE else View.VISIBLE
        requestLayout()
    }

    private fun standardMobileTypeLayoutParams(): LinearLayout.LayoutParams {
        val params =
            LinearLayout.LayoutParams(
                ViewGroup.LayoutParams.WRAP_CONTENT,
                mobileTypeContainerHeight(),
            )
        params.gravity = Gravity.CENTER_VERTICAL
        if (NewStatusBarIcons.isEnabled) {
            params.marginStart = 0
            params.marginEnd =
                resources.getDimensionPixelSize(R.dimen.status_bar_mobile_type_container_margin_end)
        } else {
            params.marginStart = spToPx(2.5f)
            params.marginEnd = spToPx(1f)
        }
        return params
    }
    ...
    private fun compactMobileTypeHeight(): Int = (standardMobileTypeHeight() / 2).coerceAtLeast(1)

    private fun mobileSignalHeight(): Int =
        resources.getDimensionPixelSize(
            if (NewStatusBarIcons.isEnabled) {
                R.dimen.status_bar_mobile_signal_size_updated
            } else {
